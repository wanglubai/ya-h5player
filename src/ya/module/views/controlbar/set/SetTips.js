...
class SetTips extends CountTips {
  constructor() {
    ...
    var dom =
      ...
      "<div class='ya-set-continer2'><div class='ya-set-title3'>弹幕位置</div><div class='ya-set-location ya-set-location0'></div><div class='ya-set-location ya-set-location1'></div><div class='ya-set-location ya-set-location2'></div></div>" +
  ...
  _addEvent_() {
    ...
    for (let i = 0; i < 3; i++) {
      this.$find(".ya-set-location" + i).on("click", e =>
        this._clickLocationCall(i)
      );
    }
    for (let i = 0; i < 3; i++) {
  ...
  _clickLocationCall(index) {
    if (!this.$find(".ya-set-location" + index).hasClass("active")) {
      this.setLocation(index);
      this.emit({ type: SetTips.ChangeLocation, value: index });
    }
  }
  ...
  setLocation(index) {
    this.$find(".ya-set-location").removeClass("active");
    this.$find(".ya-set-location" + index).addClass("active");
  }
  setDensity(index) {
...
SetTips.ChangeLocation = "SetTips.ChangeLocation";
